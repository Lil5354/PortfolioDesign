...
| ID | Vấn đề cụ thể | Mô tả hiện trạng | Đầu ra mong muốn / Flow đúng | Nguyên nhân & Fix (Lần 1) | Nguyên nhân & Fix (Lần 2) | Nguyên nhân & Fix (Lần 3) |
|---|---|---|---|---|---|---|
| **2.1** | **Đồng bộ TOÀN BỘ dữ liệu User (Tác giả)** | **Đạt (Pass)** - Đã sửa lỗi crash/null khi view chi tiết. | Tất cả các thông tin của User (Avatar, Tên họ, Chi tiết Portfolio...) phải hiển thị đầy đủ ở mọi nơi. Tuyệt đối không được rỗng/NULL. | **Nguyên nhân:** Các query lấy danh sách Artwork (`ArtworksController`) quên include bảng `User` hoặc thiếu select trường `AvatarUrl`, `PortfolioSettings`. **Fix:** ĐÃ FIX (Thêm `Include(PortfolioSettings)` và select vào object DTO). | | |
...
| **2.3** | **Đồng bộ Dữ liệu Tương tác (Like/View)** | **Đang theo dõi** - Chưa refactor Global Store. | Số Like, lượt Xem khi thay đổi ở 1 nơi phải cập nhật ngay ở các trang khác (Gallery, Profile) và đồng bộ về dtb. | **Nguyên nhân:** FE không chia sẻ global state (vd: Zustand/React Query), mỗi component tự ôm state cũ. **Fix:** NOTE (Tạm gác lại. Yêu cầu refactor lớn toàn bộ `portfolio_system.jsx` để đưa state vào Global Store bằng Zustand, tránh làm hỏng luồng render hiện tại). | | |
| **2.4** | **Đồng bộ Huy hiệu cá nhân/ấn phẩm** | **Đạt (Pass)** - Huy hiệu hiển thị chuẩn ở mọi view. | Các huy hiệu đã gán phải xuất hiện đồng bộ ở mọi chức năng liên quan (Gallery, chi tiết, tài khoản). | **Nguyên nhân:** Endpoint `GetArtwork(id)` và `GetUsers()` chưa include bảng mapping `ArtworkBadges` / `UserAccountBadge`. **Fix:** ĐÃ FIX (Bổ sung logic join/include bảng badge vào query chi tiết ở Users và Admin Controller). | | |
