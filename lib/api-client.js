...
export const api = {
  ...
  users: {
    ...
    searchMentions(q) {
      return fetchJSON(`/users/search-mentions?q=${encodeURIComponent(q)}`);
    },
    myArtworks() {
  ...
  admin: {
    ...
    artworks(params = {}) {
      const q = new URLSearchParams(params).toString();
      return fetchJSON(`/admin/artworks${q ? `?${q}` : ""}`).then(res => {
        if (typeof window !== 'undefined' && window.MOCK_PROJECTS) {
           let mocks = window.MOCK_PROJECTS;
           if (params.q) {
             mocks = mocks.filter(p => p.title.toLowerCase().includes(params.q.toLowerCase()));
           }
           res.artworks = [...mocks, ...(res.artworks || [])];
        }
        return res;
      });
    },
    ...
    setArtworkStatus(id, isPublic, status = null) {
      return fetchJSON(`/admin/artworks/${id}/status`, { method: "PATCH", body: JSON.stringify({ isPublic, status }) });
    },
