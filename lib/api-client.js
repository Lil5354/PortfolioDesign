...
export const api = {
  artworks: {
    ...
    // Mock function to simulate AI Analysis for Intellectual Property
    analyzeArtworkWithAI(fileOrUrl) {
      return new Promise((resolve) => {
        setTimeout(() => {
          // Simulate some variance
          const originalityScore = Math.floor(Math.random() * 30) + 70; // 70 to 100
          const aiGeneratedPercentage = Math.floor(Math.random() * 20); // 0 to 20
          
          resolve({
            originalityScore,
            aiGeneratedPercentage,
            isAiVerified: originalityScore >= 75 && aiGeneratedPercentage <= 15,
            analysisDetails: "The AI engine has scanned the image against known databases. No significant matches found for direct plagiarism. Stylistic analysis suggests human-generated brush strokes/vectors."
          });
