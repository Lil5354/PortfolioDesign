...
const insertIndex = content.indexOf('function BadgesPage');
if (insertIndex !== -1) {
    content = content.slice(0, insertIndex) + componentCode + content.slice(insertIndex);
    fs.writeFileSync(file, content);
    console.log("Successfully inserted AdminUserBadgesPage");
} else {
    console.log("Could not find function BadgesPage");
}
